...
import { CalendarDays, Search, Filter, CheckCircle, XCircle, FileCheck, Check, X, Loader2 } from 'lucide-react';
...
export function LeaveManagement() {
  ...
  const handleUpdate = async (e, status) => {
    ...
    try {
      await api.patch(`/leaves/${selectedLeave._id}`, { status, remarks });
      loadLeaves();
  ...
  const filteredLeaves = leaves.filter(l => {
    const studentName = (l.studentId?.name || l.studentName || '').toLowerCase();
    const studentCustomId = (l.studentId?.customId || l.studentId || '').toString().toLowerCase();
              ...
              <div key={l._id} className={cn(
                ...
                <div className="bg-background/80 p-4 rounded-xl border border-border/50 mb-4 space-y-3 flex-grow">
                  <div className="flex justify-between items-center pb-3 border-b border-border/30">
                    <div>
                      <p className="font-bold text-sm text-foreground">{l.studentId?.name || l.studentName || 'Unknown Student'}</p>
                      <p className="text-[10px] text-muted-foreground uppercase font-medium mt-0.5">
                        {typeof l.studentId === 'object' ? l.studentId?.customId : l.studentId} • {l.studentId?.course || 'Student'}
                      </p>
                    </div>
